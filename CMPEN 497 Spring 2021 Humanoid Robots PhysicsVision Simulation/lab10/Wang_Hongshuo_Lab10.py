...
if __name__ == '__main__':
  ...
  while True:
    ...
    my_img = np.array(img * 255, dtype='uint8')
    face = model.detectMultiScale(my_img, 1.1, 6)
    print(face)
    if (face != ()):

      upper_left_x, upper_left_y, width, height = face[0]



    #upper_left_x, upper_left_y, width, height = face[0]
    top_left = (upper_left_x,upper_left_y)
    bottom_right = (upper_left_x+width,upper_left_y+width)
    ...
    # Uncomment this line and replace "faces" with the name of the list storing your faces
    # cv2.putText(out, 'Faces detected: %d'%len(faces), (4, 28), font, font_sc, font_clr, lineType=16)
    if face != ():
      out = cv2.rectangle(out, top_left, bottom_right, (255,0,0), width, 1)
    # Resize final output
